...
#define USART_SEND_LEN USART_REC_LEN /* 定义最大接收字节数 200 */
...
extern uint8_t g_usart3_rx_buf[USART_REC_LEN]; /* 接收缓冲,最大USART_REC_LEN个字节.末字节为换行符 */
extern uint16_t g_usart3_rx_index;             /* 接收状态标记 */
extern uint8_t g_uart3_rx_byte;

void usart_init(uint32_t bound); /* 串口初始化函数 */
bool GetLoraPresentFlag(void); /* 检查LoRa模块是否在位 */
