...
static volatile bool g_lora_module_present; // LoRa模块在位状态
...
uint8_t g_usart_rx_buf[USART_REC_LEN] = {0}; /* 接收缓冲, 最大USART_REC_LEN个字节. */
uint16_t g_usart_rx_sta = 0;
uint8_t g_rx_buffer[RXBUFFERSIZE] = {0};

uint8_t g_usart3_rx_buf[USART_REC_LEN] = {0}; /* 接收缓冲, 最大USART_REC_LEN个字节. */
uint16_t g_usart3_rx_index = 0;
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    else if (huart->Instance == USART3) /* 如果是串口3，进行串口3 MSP初始化 */
    {
        ...
        LORA_RX_CLOCK_ENABLE();
        LORA_TX_CLOCK_ENABLE();

        HAL_NVIC_EnableIRQ(USART3_IRQn);         /* 使能串口3中断通道 */
        HAL_NVIC_SetPriority(USART3_IRQn, 3, 3); /* 组2，最低优先级:抢占优先级3，子优先级3 */

        // 初始化LORA_TX_PIN
        gpio_init_struct.Pin = LORA_TX_PIN;
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;       /* 复用推挽输出 */
        gpio_init_struct.Pull = GPIO_PULLUP;           /* 上拉 */
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH; /* IO速度设置为高速 */
        HAL_GPIO_Init(LORA_TX_PORT, &gpio_init_struct);

        // 配置LoRa接收引脚为复用输入模式
        gpio_init_struct.Pin = LORA_RX_PIN;
        gpio_init_struct.Mode = GPIO_MODE_AF_INPUT;
        HAL_GPIO_Init(LORA_RX_PORT, &gpio_init_struct);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        // 存储接收到的字节到接收缓冲区
        if (g_usart3_rx_index < USART_REC_LEN - 1)
        {
            g_usart3_rx_buf[g_usart3_rx_index++] = g_uart3_rx_byte;
        }
        else
        {
            g_usart3_rx_index = 0; // 缓冲区溢出，重新开始接收
        }

        // 检测是否收到"OK"响应
        if (g_usart3_rx_index >= 2 &&
            g_usart3_rx_buf[g_usart3_rx_index - 2] == 'O' &&
            g_usart3_rx_buf[g_usart3_rx_index - 1] == 'K')
        {
            g_lora_module_present = true; // 设置模块在位状态为TRUE
            memset(g_usart3_rx_buf, 0, g_usart3_rx_index);
            g_usart3_rx_index = 0;
        }
...
{
    HAL_UART_IRQHandler(&g_uart1_handle); /* 调用HAL库的串口1中断处理函数 */
}

void USART3_IRQHandler(void)
{
    HAL_UART_IRQHandler(&g_uart3_handle); /* 调用HAL库的串口3中断处理函数 */
}

// 返回模块在位状态
bool GetLoraPresentFlag(void)
{
    return g_lora_module_present;
}
